    ...
    async function (request, reply) {
      try {
        ...
        if(useEp) {
          ...
          const existingWhitelistRecord = await server.whitelistRepository.findOneByApiKeyPolicyIdAndEpVersion(api_key, EPVersions.EPV_06, policyId);
    ...
    async function (request, reply) {
      try {
        ...
        if(useEp) {
          ...
          const existingWhitelistRecord = await server.whitelistRepository.findOneByApiKeyPolicyIdAndEpVersion(
            api_key,
            EPVersions.EPV_06,
            policyId
          );
  ...
  server.post("/removeWhitelist/v2",
    async function (request, reply) {
      try {
        ...
        const validAddresses = address.every(ethers.utils.isAddress);
        if (!validAddresses) return reply.code(ReturnCode.FAILURE).send({ error: ErrorMessage.INVALID_ADDRESS_PASSSED });
        const existingWhitelistRecord = await server.whitelistRepository.findOneByApiKeyPolicyIdAndEpVersion(api_key, EPVersions.EPV_07, policyId);
