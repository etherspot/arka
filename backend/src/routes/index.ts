...
const routes: FastifyPluginAsync = async (server) => {
  const paymaster = new Paymaster(
    server.config.FEE_MARKUP,
    server.config.MULTI_TOKEN_MARKUP
  );
    ...
    try {
      ...
      if (!body)
        return reply
          ...
          .send({ error: ErrorMessage.EMPTY_BODY });
      ...
      if (body.method) {
        switch (body.method) {
          ...
          default: {
            return reply
              .code(ReturnCode.FAILURE)
              .send({ error: ErrorMessage.UNSUPPORTED_METHOD });
            break;
          }
        }
      }
      if (!api_key)
        return reply
          .code(ReturnCode.FAILURE)
          .send({ error: ErrorMessage.INVALID_API_KEY });
      console.log("entryPoint: ", entryPoint);
      if (
        entryPoint != SUPPORTED_ENTRYPOINTS.EPV_06 &&
        entryPoint != SUPPORTED_ENTRYPOINTS.EPV_07
      )
        return reply
          .code(ReturnCode.FAILURE)
          .send({ error: ErrorMessage.UNSUPPORTED_ENTRYPOINT });
      ...
      if (!unsafeMode) {
        ...
        if (secrets["MULTI_TOKEN_PAYMASTERS"]) {
          const buffer = Buffer.from(
            secrets["MULTI_TOKEN_PAYMASTERS"],
            "base64"
          );
        ...
        indexerEndpoint =
          secrets["INDEXER_ENDPOINT"] ?? process.env.DEFAULT_INDEXER_ENDPOINT;
      } else {
        const record: any = await getSQLdata(
          api_key,
          server.sqlite.db,
          server.log
        );
        ...
        indexerEndpoint =
          record["INDEXER_ENDPOINT"] ?? process.env.DEFAULT_INDEXER_ENDPOINT;
      }

      if (!userOp || !entryPoint || !chainId || !mode || isNaN(chainId)) {
        server.log.info("Incomplete body data provided");
        return reply
          .code(ReturnCode.FAILURE)
          .send({ error: ErrorMessage.INVALID_DATA });
      }

      if (server.config.SUPPORTED_NETWORKS == "" && !SupportedNetworks) {
        return reply
          .code(ReturnCode.FAILURE)
          .send({ error: ErrorMessage.UNSUPPORTED_NETWORK });
      }
      ...
      if (
        ...
        return reply
          .code(ReturnCode.FAILURE)
          .send({ error: ErrorMessage.UNSUPPORTED_NETWORK_TOKEN });

      if (gasToken && ethers.utils.isAddress(gasToken))
        gasToken = ethers.utils.getAddress(gasToken);

      if (
        mode.toLowerCase() == "multitoken" &&
        !(
          multiTokenPaymasters[chainId] &&
          multiTokenPaymasters[chainId][gasToken]
        ) &&
      ...
      switch (mode.toLowerCase()) {
        case "sponsor": {
          ...
          if (txnMode) {
            ...
            const IndexerData = await getIndexerData(
              signerAddress,
              userOp.sender,
              date.getMonth(),
              date.getFullYear(),
            ...
            if (IndexerData.length >= noOfTxns)
              return reply
                .code(ReturnCode.FAILURE)
                .send({ error: ErrorMessage.QUOTA_EXCEEDED });
          }
          const validUntil = context?.validUntil
            ? new Date(context.validUntil)
            : date;
          const validAfter = context?.validAfter
            ? new Date(context.validAfter)
            : date;
          const hex = (
            Number((validUntil.valueOf() / 1000).toFixed(0)) + 600
          ).toString(16);
          const hex1 = (
            Number((validAfter.valueOf() / 1000).toFixed(0)) - 60
          ).toString(16);
          let str = "0x";
          let str1 = "0x";
          ...
          if (entryPoint == SUPPORTED_ENTRYPOINTS.EPV_06)
            result = await paymaster.signV06(
              userOp,
              str,
              str1,
              entryPoint,
              networkConfig.contracts.etherspotPaymasterAddress,
              networkConfig.bundler,
              signer,
              estimate,
              server.log
            );
          else
            result = await paymaster.signV07(
              userOp,
              str,
              str1,
              entryPoint,
        ...
        case "erc20": {
          if (entryPoint !== SUPPORTED_ENTRYPOINTS.EPV_06)
            throw new Error(
              "Currently only 0x5FF137D4b0FDCD49DcA30c7CF57E578a026d2789 entryPoint address is supported"
            );
          let paymasterAddress: string;
          if (customPaymasters[chainId] && customPaymasters[chainId][gasToken])
            paymasterAddress = customPaymasters[chainId][gasToken];
          else paymasterAddress = PAYMASTER_ADDRESS[chainId][gasToken];
          result = await paymaster.pimlico(
            userOp,
            networkConfig.bundler,
            entryPoint,
            paymasterAddress,
        ...
        case "multitoken": {
          ...
          if (
            ...
            !(
              networkConfig.MultiTokenPaymasterOracleUsed == "orochi" ||
              networkConfig.MultiTokenPaymasterOracleUsed == "chainlink"
            )
          )
            throw new Error("Oracle is not Defined/Invalid");
          result = await paymaster.signMultiTokenPaymaster(
            userOp,
            str,
            str1,
            entryPoint,
            multiTokenPaymasters[chainId][gasToken],
            gasToken,
            multiTokenOracles[chainId][gasToken],
            networkConfig.bundler,
            signer,
            networkConfig.MultiTokenPaymasterOracleUsed,
            server.log
        ...
        default: {
          return reply
            .code(ReturnCode.FAILURE)
            .send({ error: ErrorMessage.INVALID_MODE });
        }
      }
      server.log.info(result, "Response sent: ");
      if (sponsorDetails)
        result.sponsor = { name: sponsorName, icon: sponsorImage };
      if (body.jsonrpc)
        return reply
          .code(ReturnCode.SUCCESS)
          .send({ jsonrpc: body.jsonrpc, id: body.id, result, error: null });
      ...
      return reply
        .code(ReturnCode.FAILURE)
        .send({ error: err.message ?? ErrorMessage.FAILED_TO_PROCESS });
    }
  });
      ...
      try {
        ...
        if (!privateKey)
          return reply
            .code(ReturnCode.FAILURE)
            .send({ error: ErrorMessage.INVALID_API_KEY });
        if (!entryPoint || !gasToken || !chainId || isNaN(chainId)) {
          return reply
            .code(ReturnCode.FAILURE)
            .send({ error: ErrorMessage.INVALID_DATA });
        }
        ...
        let result;
        if (customPaymasters[chainId] && customPaymasters[chainId][gasToken])
          result = { message: customPaymasters[chainId][gasToken] };
        else {
          if (
            !(
              PAYMASTER_ADDRESS[chainId] && PAYMASTER_ADDRESS[chainId][gasToken]
            ...
            return reply
              .code(ReturnCode.FAILURE)
              .send({ error: "Invalid network/token" });
  ...
  server.post("/whitelist", async function (request, reply) {
    try {
      ...
      if (!validAddresses)
        return reply
          .code(ReturnCode.FAILURE)
          .send({ error: "Invalid Address passed" });
      const result = await paymaster.whitelistAddresses(
        address,
        networkConfig.contracts.etherspotPaymasterAddress,
        networkConfig.bundler,
        privateKey,
    ...
    try {
      ...
      if (!validAddresses)
        return reply
          .code(ReturnCode.FAILURE)
          .send({ error: "Invalid Address passed" });
      const result = await paymaster.removeWhitelistAddress(
        address,
        networkConfig.contracts.etherspotPaymasterAddress,
        networkConfig.bundler,
        privateKey,
  ...
  server.post("/checkWhitelist", async function (request, reply) {
    try {
      ...
      if (!networkConfig)
        return reply
          .code(ReturnCode.FAILURE)
          .send({ error: ErrorMessage.UNSUPPORTED_NETWORK });
      const response = await paymaster.checkWhitelistAddress(
        accountAddress,
        networkConfig.contracts.etherspotPaymasterAddress,
        networkConfig.bundler,
        privateKey,
      ...
      if (body.jsonrpc)
        return reply
          ...
          .send({
            ...
            id: body.id,
            result: {
              message: response === true ? "Already added" : "Not added yet",
            },
            error: null,
          });
      return reply
  ...
  server.post(
    ...
    async function (request, reply) {
      try {
        ...
        if (isNaN(amount) || !chainId || isNaN(chainId)) {
          return reply
            .code(ReturnCode.FAILURE)
            .send({ error: ErrorMessage.INVALID_DATA });
        }
        ...
        if (!networkConfig)
          return reply
            .code(ReturnCode.FAILURE)
            .send({ error: ErrorMessage.UNSUPPORTED_NETWORK });
        return await paymaster.deposit(
          amount,
          networkConfig.contracts.etherspotPaymasterAddress,
          networkConfig.bundler,
          privateKey,
