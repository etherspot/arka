...
const AddSupportedNetworksModal = ({supportedNetworks, setSupportedNetworks, open, handleClose}) => {
  ...
  const [supportedNetworkRow, setSupportedNetworkRow] = useState(
	...
	const addRow = () => {
		if (supportedNetworkRow.bundler === "" || supportedNetworkRow.contracts.etherspotPaymasterAddress === "" || supportedNetworkRow.chainId === 0) {
			toast.error("Please fill all textfields")
			return;
		}
		if(!utils.isAddress(supportedNetworkRow.contracts.etherspotPaymasterAddress)) {
		...
		{	supportedNetworks.push(supportedNetworkRow);
			setSupportedNetworks(supportedNetworks.sort((a,b) => a.chainId - b.chainId));
	...
	const deleteRow = (chainId) => {
		setSupportedNetworks(supportedNetworks.filter((element) => element.chainId != chainId));
		setSupportedNetworkRow(defaultSupportedNetworksRow);
	}
