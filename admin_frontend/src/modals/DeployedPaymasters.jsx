...
const DeployedPaymastersModal = ({supportedNetworks, setSupportedNetworks, open, handleClose, title}) => {
  ...
  const addRow = () => {
    ...
    if (supportedNetworkRow.address === "" || supportedNetworkRow.chainId === "" || supportedNetworkRow.chainId === 0) {
    ...
    if (
      !addresses.find(
        (network) =>
          network.chainId === supportedNetworkRow.chainId && network.address === supportedNetworkRow.address
      )
    )
    {
  ...
  const deleteRow = (network) => {
    const tempAddr = addresses.filter((element) => element.chainId.toString() !== network.chainId.toString() && element.address !== network.address)
    setAddresses(tempAddr);
    supportedNetworks[network.chainId] = supportedNetworks[network.chainId].filter(element => element !== network.address.toString())
    setSupportedNetworks(supportedNetworks)
    setSupportedNetworkRow(defaultSupportedNetworksRow);
  }
